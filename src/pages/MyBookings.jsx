...
export default function MyBookings() {
  ...
  const downloadPDF = (booking) => {
    ...
    doc.setFontSize(16);
    doc.text("Travel Booking Ticket", 20, 20);

    doc.setFontSize(12);
    doc.text(`Booking ID: ${booking.bookingId || "N/A"}`, 20, 40);
    doc.text(`Name: ${booking.name}`, 20, 50);
    doc.text(`Email: ${booking.email}`, 20, 60);
    doc.text(`Destination: ${booking.destination}`, 20, 70);
    doc.text(`Date: ${booking.date}`, 20, 80);
    doc.text(`Adults: ${booking.adults}`, 20, 90);
    doc.text(`Children: ${booking.children}`, 20, 100);
      ...
      {bookings.length === 0 ? (
        ...
        bookings.map((b) => (
          <div className="booking-item" key={b.id}>
            ...
            <p><strong>Adults:</strong> {b.adults}</p>
            <p><strong>Children:</strong> {b.children}</p>
